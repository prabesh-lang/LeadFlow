...
-- Generated from prisma/schema.postgresql.prisma via `prisma migrate diff --from-empty`.
--
-- How to use:
--   1. Open Supabase Dashboard → SQL Editor → New query.
--   2. Paste this entire file and run.
--   3. Use only on an empty database or drop conflicting tables first (see below).
--
-- Notes:
--   - Supabase Auth (`auth.users`) is separate; `User.authUserId` stores the Supabase Auth user id.
--   - Prisma `cuid()` ids are plain TEXT; no UUID extension required.
--   - Table names are quoted (e.g. "User") because User is a reserved word in PostgreSQL.
--   - If you already have tables with these names, DROP them (and dependents) before running,
--     or adjust this script (e.g. different schema).
