  ...
  return (
    <div className="relative" data-dashboard-export>
      <button
        ...
        onClick={() => setOpen((o) => !o)}
        disabled={busy !== null}
        className="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-lf-bg px-3 py-2 text-sm font-medium text-lf-text shadow-sm hover:bg-white/5 disabled:opacity-60"
      >
        {busy ? (
      ...
      {open && !busy ? (
        <>
          ...
          <div
            role="menu"
            className="absolute right-0 top-full z-50 mt-1 min-w-[11rem] rounded-xl border border-white/10 bg-lf-surface py-1 shadow-xl"
          >
            <button
              type="button"
              role="menuitem"
              className="block w-full px-4 py-2.5 text-left text-sm text-lf-text hover:bg-white/5"
              onClick={() => run("pdf")}
            >
              PDF
            </button>
